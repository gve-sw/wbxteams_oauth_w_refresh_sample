...
def started():
    ...
    firstText=firstSender=firstCreated=firstRoomTitle='none'

    # then, retrieve the list of Spaces (rooms) the logged in user belongs to, sorted by last activity
    theRooms=api.rooms.list(sortBy='lastactivity',max=1)

    # now iterate through the rooms to just grab the latests one with activity
    # and from that, list messages but also just extract the latest message
    for theRoom in theRooms:
        theMessages=api.messages.list(roomId=theRoom.id,max=1)
        firstRoomTitle=theRoom.title
        for theMessage in theMessages:
            firstText=theMessage.text
            firstSender=theMessage.personEmail
            firstCreated = theMessage.created.strftime("%m/%d/%Y, %H:%M:%S")
            break
        break
    ...
    return ("""<!DOCTYPE html>
               ...
               <br>
               The most recent message in a space you belong to is from """+firstSender+""" in space titled """+firstRoomTitle+""" 
               <br>
               was sent on: """+firstCreated+"""
               <br>
               And it's content is: """+firstText+"""
               </body>
               </html>
            """)
